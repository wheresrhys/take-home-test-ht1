...
description: >-
  ...
  of GitHub issues, one per task, by reusing the flesh-out-ticket skill for each. Folds the
...
### 1. Gather & parse tasks
- Source: the skill argument (a pasted list of tasks) if given; otherwise read `tasks.md`.
- Parse into an ordered list of `{heading, text}`. Each numbered line item under a `#` heading is
  one task. The literal numbers in `tasks.md` are messy/duplicated — treat every line item as a
  task, ignore its number.
- Skip meta-process sections (e.g. `# Skills`) by default — they describe workflow, not product
  work.
- Present the parsed list back to the user (heading + a one-line summary per task) and let them
...
### 2. Draft each ticket (parallelise)
...
### 3. Confirm each ticket — HARD GATE, propagated from flesh-out-ticket
...
This gate is mandatory for every ticket. Parallel drafting in step 2 must never bypass it — the
...
### 5. Wire up dependencies (second pass)
